...
def create_footprint(r:int) -> np.array:
...
def voxel_smoothing(masks_arr:np.array, opening_footprint_radius:int, closing_to_opening_ratio:float) -> np.array: 
    footprint_small = create_footprint(r=int(opening_footprint_radius*closing_to_opening_ratio))
    closed_masks_arr = morphology.closing(masks_arr, footprint_small) 
    footprint = create_footprint(r=opening_footprint_radius)
